...
"""Compare C++ and Python repeated bounded reconstruction boundaries."""
...
def main() -> int:
    ...
    for vector, expected in zip(source["vectors"], fixed["cases"], strict=True):
        completed = subprocess.run(
            [
                str(args.binary.resolve()), "bounded-reconstruct-repeated", vector["seed"],
        ...
        expected_document = {
            ...
            **dict(zip(fixed["counter_fields"], expected["counters"], strict=True)),
            ...
            "first_reconstruction": dict(zip(
                fixed["reconstruction_fields"], expected["first"], strict=True
            )),
            "last_reconstruction": dict(zip(
                fixed["reconstruction_fields"], expected["last"], strict=True
            )),
            "exhaustion": dict(zip(
                fixed["exhaustion_fields"], expected["exhaustion"], strict=True
            )),
            "digest": None,
        ...
        if cpp != python or python != expected_document:
            ...
            print(f"PASS repeated-reconstruction {vector['name']}")
    if failures:
        raise AssertionError("C++/Python repeated reconstruction mismatch: " + ", ".join(failures))
    print(f"All {len(source['vectors'])} repeated reconstruction vectors passed")
    return 0
