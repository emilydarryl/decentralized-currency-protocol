...
"""Tests for deterministic hierarchical regeneration checkpoints."""
...
from contrib.pow_research_v1.checkpoint_recursive_regeneration import (
    ...
    reconstruct_repeatedly_with_hierarchical_checkpoint_ladder,
)
from contrib.pow_research_v1.powvm import prepare_epoch


ROOT = Path(__file__).resolve().parents[2]
FIXED = ROOT / "contrib" / "pow_research_v1" / "vectors" / "hierarchical_checkpoint_ladder_v0.json"


class HierarchicalCheckpointLadderV1Test(unittest.TestCase):
    def test_fixed_boundaries_and_exact_accounting(self) -> None:
        ...
        expected_levels = [
            {"level": level, "stride": stride, "capacity": capacity, "base_slot": base_slot}
            for level, stride, capacity, base_slot in HIERARCHICAL_CHECKPOINT_LEVELS
        ]
        self.assertEqual(expected_levels, fixed["hierarchical_checkpoint_levels"])

        for expected in fixed["cases"]:
            result = reconstruct_repeatedly_with_hierarchical_checkpoint_ladder(
            ...
            self.assertEqual(
                result["hierarchical_checkpoint_levels"],
                fixed["hierarchical_checkpoint_levels"],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                result["checkpoint_lookups"] * len(HIERARCHICAL_CHECKPOINT_LEVELS),
            )
