...
"""Compare independent C++ and Python operation-bounded bundle results."""
    ...
    for vector in source["vectors"]:
        completed = subprocess.run(
            [
                ...
                "recursive-regenerate-operation-bounded-bundle",
                vector["seed"],
                vector["header"],
                str(vector["nonce"]),
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_operation_bounded_dependency_bundles(
        ...
        if cpp != python:
            ...
            print(
                f"C++ mismatch for {vector['name']}:\n"
                f"{json.dumps(cpp, indent=2)}"
            )
        else:
            print(f"PASS operation-bounded-dependency-bundle {vector['name']}")
    if failures:
        raise AssertionError(
            "C++/Python operation-bounded bundle mismatch: "
            + ", ".join(failures)
        )
    print(f"All {len(source['vectors'])} operation-bounded bundle vectors passed")
    return 0
