...
"""Verify canonical v1 outputs through the no-spill recomputation backend."""
...
def main() -> int:
    ...
    for vector in document["vectors"]:
        ...
        if actual != expected:
            ...
            print(f"PASS half-recompute {vector['name']}")
    if failures:
        raise AssertionError("C++ v1 recomputation differential failures:\n" + "\n".join(failures))
    print(f"All {len(document['vectors'])} C++ v1 recomputation vectors passed")
    return 0
