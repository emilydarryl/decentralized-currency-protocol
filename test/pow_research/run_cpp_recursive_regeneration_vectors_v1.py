...
"""Compare independent C++ and Python recursive-regeneration boundaries."""
...
def main() -> int:
    ...
    for vector, expected in zip(source["vectors"], fixed["cases"], strict=True):
        ...
        completed = subprocess.run(
            [str(args.binary.resolve()), "recursive-regenerate-first", vector["seed"],
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_first_recursively(
        ...
        if cpp != python:
            ...
            print(f"PASS recursive-regeneration {vector['name']}")
    if failures:
        raise AssertionError("C++/Python recursive regeneration mismatch: " + ", ".join(failures))
    print(f"All {len(source['vectors'])} recursive regeneration vectors passed")
    return 0
