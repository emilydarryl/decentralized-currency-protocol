...
"""Compare C++ and Python indexed-gap reconstruction boundaries."""
...
def main() -> int:
    ...
    for vector, expected in zip(source["vectors"], fixed["cases"], strict=True):
        completed = subprocess.run(
            [str(args.binary.resolve()), "bounded-reconstruct-indexed-gap", vector["seed"],
        ...
        if cpp != python or python != expected_document:
            ...
            print(f"PASS indexed-gap-reconstruction {vector['name']}")
    if failures:
        raise AssertionError("C++/Python indexed-gap reconstruction mismatch: " + ", ".join(failures))
    print(f"All {len(source['vectors'])} indexed-gap reconstruction vectors passed")
    return 0
