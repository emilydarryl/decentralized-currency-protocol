...
"""Compare independent C++ and Python target-checkpoint results."""
...
def main() -> int:
    ...
    for vector in source["vectors"]:
        completed = subprocess.run(
            [str(args.binary.resolve()), "recursive-regenerate-target-checkpoint",
             vector["seed"], vector["header"], str(vector["nonce"]),
             str(params.dataset_bytes), str(params.scratchpad_bytes), str(params.passes)],
            check=True, capture_output=True, text=True,
        )
        cpp = json.loads(completed.stdout)
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_target_checkpoints(
        ...
        if cpp != python:
            ...
            print(f"PASS target-checkpoint-regeneration {vector['name']}")
    if failures:
        raise AssertionError("C++/Python target-checkpoint mismatch: " + ", ".join(failures))
    print(f"All {len(source['vectors'])} target-checkpoint vectors passed")
    return 0
