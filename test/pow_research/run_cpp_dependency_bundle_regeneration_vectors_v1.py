...
"""Compare independent C++ and Python dependency-bundle results."""
    ...
    for vector in source["vectors"]:
        completed = subprocess.run(
            [
                ...
                "recursive-regenerate-dependency-bundle",
                vector["seed"],
                vector["header"],
                str(vector["nonce"]),
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_dependency_bundles(
        ...
        if cpp != python:
            ...
            print(f"PASS dependency-bundle-regeneration {vector['name']}")
    if failures:
        raise AssertionError(
            "C++/Python dependency-bundle mismatch: " + ", ".join(failures)
        )
    print(f"All {len(source['vectors'])} dependency-bundle vectors passed")
    return 0
