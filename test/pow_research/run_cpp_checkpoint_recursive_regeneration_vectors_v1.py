...
"""Compare independent C++ and Python checkpoint-regeneration results."""
...
def main() -> int:
    ...
    for vector in source["vectors"]:
        completed = subprocess.run(
            [str(args.binary.resolve()), "recursive-regenerate-checkpoint", vector["seed"],
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_checkpoints(
        ...
        if cpp != python:
            ...
            print(f"PASS checkpoint-recursive-regeneration {vector['name']}")
    if failures:
        raise AssertionError("C++/Python checkpoint mismatch: " + ", ".join(failures))
    print(f"All {len(source['vectors'])} checkpoint regeneration vectors passed")
    return 0
