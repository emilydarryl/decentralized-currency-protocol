...
"""Verify canonical v1 outputs through the exact half-spill backend."""
...
def main() -> int:
    ...
    failures: list[str] = []
    with tempfile.TemporaryDirectory(prefix="soveroot-vector-half-spill-") as directory:
        for vector in document["vectors"]:
            ...
            if actual != expected:
                ...
                print(f"PASS half-spill {vector['name']}")

    if failures:
        raise AssertionError("C++ v1 half-spill differential failures:\n" + "\n".join(failures))
    print(f"All {len(document['vectors'])} C++ v1 half-spill vectors passed")
    return 0
