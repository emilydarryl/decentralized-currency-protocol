...
"""Compare C++ and Python iterative work-stack refusal boundaries."""
    ...
    for expected in fixed["cases"]:
        ...
        completed = subprocess.run(
            [
                ...
                "recursive-regenerate-iterative-work-stack",
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_iterative_work_stack(
        ...
        if cpp != python:
            ...
            print(f"PASS iterative-work-stack seed {seed_index}")
    if failures:
        raise AssertionError(
            "C++/Python iterative work-stack mismatch: "
            + ", ".join(str(item) for item in failures)
        )
    print(f"All {len(fixed['cases'])} iterative work-stack vectors passed")
    return 0
