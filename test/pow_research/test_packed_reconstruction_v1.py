...
"""Tests for bitmap-ranked packed replay reconstruction."""
...
from contrib.pow_research_v1.packed_reconstruction import reconstruct_with_packed_checkpoints
from contrib.pow_research_v1.powvm import Params, prepare_epoch


ROOT = Path(__file__).resolve().parents[2]
SOURCE = ROOT / "contrib" / "pow_research_v1" / "vectors" / "v1.json"
FIXED = ROOT / "contrib" / "pow_research_v1" / "vectors" / "packed_reconstruction_v0.json"


class PackedReconstructionV1Test(unittest.TestCase):
    def test_smoke_extends_flat_reconstruction_boundary(self) -> None:
        params, _ = profile("smoke")
        result = reconstruct_with_packed_checkpoints(
            prepare_epoch(seed_for(0), params), HEADER, 0
        )
        self.assertEqual(result.status, "refused_packed_checkpoint_exhausted")
        self.assertEqual(result.reconstructed_misses, 46)
        self.assertEqual(result.completed_iterations, 183)
        self.assertEqual(result.layout.replay_value_capacity, 312)
        self.assertTrue(result.all_replay_states_matched)
        self.assertEqual(result.max_replay_peak_entries, result.layout.replay_value_capacity)
        self.assertGreater(result.cumulative_shifted_bytes, 0)
        self.assertIsNone(result.execution_result)

    def test_packed_result_is_deterministic(self) -> None:
        params, _ = profile("smoke")
        context = prepare_epoch(seed_for(1), params)
        self.assertEqual(
            reconstruct_with_packed_checkpoints(context, HEADER, 1 << 32),
            reconstruct_with_packed_checkpoints(context, HEADER, 1 << 32),
        )
