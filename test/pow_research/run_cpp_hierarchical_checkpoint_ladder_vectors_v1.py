...
"""Compare C++ and Python hierarchical checkpoint-ladder boundaries."""
    ...
    for expected in fixed["cases"]:
        ...
        completed = subprocess.run(
            [
                ...
                "recursive-regenerate-hierarchical-checkpoint-ladder",
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_hierarchical_checkpoint_ladder(
        ...
        if cpp != python:
            ...
            print(f"PASS hierarchical-checkpoint-ladder seed {seed_index}")
    if failures:
        raise AssertionError(
            "C++/Python hierarchical checkpoint-ladder mismatch: "
            + ", ".join(str(item) for item in failures)
        )
    print(f"All {len(fixed['cases'])} hierarchical checkpoint-ladder vectors passed")
    return 0
