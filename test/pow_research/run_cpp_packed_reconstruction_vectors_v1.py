...
"""Compare C++ and Python packed-checkpoint reconstruction boundaries."""
...
def main() -> int:
    ...
    for vector, expected in zip(source["vectors"], fixed["cases"], strict=True):
        completed = subprocess.run(
            [str(args.binary.resolve()), "bounded-reconstruct-packed", vector["seed"],
        ...
        if cpp != python or python != expected_document:
            ...
            print(f"PASS packed-reconstruction {vector['name']}")
    if failures:
        raise AssertionError("C++/Python packed reconstruction mismatch: " + ", ".join(failures))
    print(f"All {len(source['vectors'])} packed reconstruction vectors passed")
    return 0
