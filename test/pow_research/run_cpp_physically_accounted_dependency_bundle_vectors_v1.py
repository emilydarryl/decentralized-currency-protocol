...
"""Compare C++ and Python physically accounted bundle boundaries."""
...
def main() -> int:
    ...
    for expected in fixed["cases"]:
        ...
        for key in ("format", "warning", "params", "nonce"):
            cpp.pop(key)
        python = reconstruct_repeatedly_with_physically_accounted_dependency_bundles(
        ...
        if cpp != python:
            ...
            print(f"PASS physically-accounted-bundle seed {seed_index}")
    if failures:
        raise AssertionError(
            "C++/Python physically accounted mismatch: "
            + ", ".join(str(item) for item in failures)
        )
    print(f"All {len(fixed['cases'])} physically accounted vectors passed")
    return 0
