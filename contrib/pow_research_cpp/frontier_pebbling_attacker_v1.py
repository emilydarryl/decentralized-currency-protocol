...
"""Collect frozen cost-aware frontier-pebbling evidence for PoW v1."""
...
FORMAT = "soveroot-pow-v1-frontier-pebbling-attacker-matrix-v0"
...
def build_matrix(
    ...
    for seed_index in range(seed_start, seed_start + seeds):
    ...
    return {
        "format": FORMAT,
        "warning": "NON-CONSENSUS FRONTIER-PEBBLING ATTACK SCREEN; no gate assessment",
...
def main() -> int:
    ...
    parser.add_argument("--seed-start", type=int, default=8)
    ...
    try:
        document = build_matrix(
            args.profile, args.seed_start, args.seeds, args.operation_limit
        )
