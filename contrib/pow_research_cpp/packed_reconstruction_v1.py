...
"""Collect packed-checkpoint reconstruction evidence for PoW v1."""
...
FORMAT = "soveroot-pow-v1-packed-checkpoint-reconstruction-matrix-v0"


def build_matrix(profile_name: str, seeds: int | None = None, source_revision: str = "unrecorded") -> dict[str, object]:
    ...
    for seed_index in range(seed_count):
        ...
        cases.append({"seed_index": seed_index, "nonce": seed_index << 32,
                      "wall_ns": time.perf_counter_ns() - started, **result})
    return {
        "format": FORMAT,
        "warning": "NON-CONSENSUS PACKED CHECKPOINT PILOT; no completed proof, throughput, or gate assessment",
        "profile": profile_name,
        "source_revision": source_revision,
        "params": params.to_dict(),
        "seed_count": seed_count,
        "all_runs_reconstructed_repeatedly": all(
            case["reconstructed_misses"] > 1 and case["all_replay_states_matched"] for case in cases
        ),
        "all_runs_exhausted_without_digest": all(
            case["status"] == "refused_packed_checkpoint_exhausted"
            and case["execution_result"] is None for case in cases
        ),
        "memory_scope": {
