...
The independent attack interface is frozen in [`external_attack_challenge_v0.json`](external_attack_challenge_v0.json) and explained in [the external attack-challenge guide](../../docs/pow-v1-external-attack-challenge.md). [`external_attack_challenge.py`](external_attack_challenge.py) issues deterministic case sets, validates manifests and result envelopes, and independently checks claimed canonical outputs. It never executes submitted code and explicitly cannot prove physical-memory eligibility. Start from the [`submission`](external_attack_submission_template_v0.json) and [`results`](external_attack_results_template_v0.json) templates; the committed public preflight is [`vectors/external_attack_qualification_v0.json`](vectors/external_attack_qualification_v0.json).
