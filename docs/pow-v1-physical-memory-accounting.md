...
The Linux C++ build emits GCC `-fstack-usage` records. CI fails unless `RecursiveRegenerator::ValueAt` has statically bounded stack use no greater than the 2,048-byte per-frame allowance. The arena vector also refuses if its allocated capacity exceeds its requested bytes plus the allocator allowance.
