...
Stage C now has a fail-closed online probe, flat replay and dense-layout pilots, a time-checkpoint screen, recursive regeneration, target-aware dependency bundles, a total-operation bound, and [physical-memory accounting](pow-v1-physical-memory-accounting.md). The latest mode allocates 85,504 arena bytes, reserves 40,960 native-stack bytes and 4,096 allocator bytes, and keeps its transcript fixed at 48 bytes. Its eight-seed maximum is 853 and no proof is produced. The immediate implementation milestone is replacing native recursion with an explicit preallocated work stack, then continuing exact attack search. Controlled-host measurement becomes gate-relevant only if an attacker emits canonical proofs.
