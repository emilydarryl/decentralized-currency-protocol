...
In plain language, we are trying to attack our own mining design using a miner that keeps only half the normal scratch memory. State-only [checkpoint-assisted regeneration](pow-v1-checkpoint-recursive-regeneration.md) regressed, [target-aware checkpoints](pow-v1-target-checkpoint-regeneration.md) tied the prior 999 record, and replay-only [direct-dependency bundles](pow-v1-dependency-bundle-regeneration.md) briefly moved seed zero to 1,006 while exposing an unbounded-work gap. The [total-operation follow-up](pow-v1-operation-bounded-dependency-bundle-regeneration.md) charges calls, replay steps, memo probes, and checkpoint probes together. All eight seeds stop at exactly five million operations, reach only iterations 480 through 999 of 98,304, and produce no proof. This is adversarial evidence, not a passed test: smarter attacks, physical-memory measurements, and hardware comparisons remain outstanding. See [Mining Decentralization in Plain English](mining-decentralization-in-plain-english.md) for the full nontechnical explanation.
...
Stage C now has a fail-closed online probe, flat replay and dense-layout pilots, a time-checkpoint screen, first and [repeated recursive regeneration](pow-v1-repeated-recursive-regeneration.md), a [checkpoint-assisted rejection pilot](pow-v1-checkpoint-recursive-regeneration.md), [target-aware checkpoints](pow-v1-target-checkpoint-regeneration.md), [direct-dependency bundles](pow-v1-dependency-bundle-regeneration.md), and a [total-operation bound](pow-v1-operation-bounded-dependency-bundle-regeneration.md). The combined bound completes the eight-seed holdout and removes the replay-only bundle record: the bounded maximum is 999, the minimum is 480, and no proof is produced. The immediate milestone is explicit accounting for every physical stack byte, allocator allowance, and peak resident byte, followed by controlled-host measurement. A later exact attacker must still account for every retained value, identity, version, checkpoint, register, queue, and transient byte within the ceiling.
