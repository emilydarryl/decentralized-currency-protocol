...
| Template autonomy | **OPEN** | Two independently written miners agree byte for byte on the authenticated message payloads, template commitment, and solved block; each declares, solves, and directly publishes a labnet block. A noncustodial payout path and adversarial multi-coordinator switching remain unproved. |
