...
Stage C now has a [fail-closed online bounded probe](pow-v1-online-bounded-probe.md) and a [bounded first-reconstruction result](pow-v1-bounded-first-reconstruction.md). The latter reconstructs one missing value from an exact sparse prefix replay, requires independently matching historical registers and accumulator, retries canonical execution, and then refuses without output at the next miss. The immediate milestone is repeated reconstruction under the same arena. A later exact attacker must account for retained values, identities, versions, checkpoints, register state, work queues, stack, allocator overhead, and peak resident memory within the ceiling.
