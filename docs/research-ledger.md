...
| Template autonomy | **OPEN** | Two independently written miners agree byte for byte and directly publish. One running miner preserves exact templates through five hostile coordinator behaviors, ordered alternate failover, and total coordinator loss. Two local receipt replicas recover from one outage, agree on a deterministic plan, and drive direct coinbase payouts. A globally ordered sharechain, independent networked implementation, Sybil resistance, and adversarial public-network evidence remain unproved. |
