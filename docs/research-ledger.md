...
Stage C now has a fail-closed online probe, first and repeated reconstruction results, a [packed checkpoint pilot](pow-v1-packed-checkpoint-reconstruction.md), and a [paged-gap pilot](pow-v1-paged-gap-reconstruction.md). Fixed 32-value pages reduce seed-zero charged insertion movement from 180.8 GB to 183.3 MB, approximately 986 times less. The tradeoff is earlier exhaustion at 6,667 occupied values because of fragmentation and 489.8 million probes from linear page lookup. The immediate milestone is a byte-accounted page-count index with bounded gap rebalancing. A later exact attacker must account for retained values, identities, versions, checkpoints, register state, work queues, stack, allocator overhead, and peak resident memory within the ceiling.
