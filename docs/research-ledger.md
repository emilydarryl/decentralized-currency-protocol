...
Stage C now has a fail-closed online probe, flat replay and dense-layout pilots, a time-checkpoint screen, first and [repeated recursive regeneration](pow-v1-repeated-recursive-regeneration.md), a [checkpoint-assisted rejection pilot](pow-v1-checkpoint-recursive-regeneration.md), [target-aware checkpoints](pow-v1-target-checkpoint-regeneration.md), and [direct-dependency bundles](pow-v1-dependency-bundle-regeneration.md). Twelve four-value bundles displace 32 memo entries yet move the seed-zero prefix from the matched 951 baseline to 1,006 and lower maximum depth from five to three. This is seven iterations beyond the prior global record, but the partial holdout contains a regression and an operator-stopped long-running case. Every case still refuses without a proof. The immediate milestone is a deterministic total-operation ceiling covering replay iterations, recursive calls, memo probes, and checkpoint probes. A later exact attacker must still account for every retained value, identity, version, checkpoint, register, queue, physical stack byte, allocator allowance, and peak resident byte within the ceiling.
