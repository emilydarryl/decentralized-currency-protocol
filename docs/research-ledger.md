...
Stage C now has a fail-closed online probe, flat replay and dense-layout pilots, a time-checkpoint screen, recursive regeneration, target-aware dependency bundles, a total-operation bound, physical-memory accounting, and an [explicit iterative work stack](pow-v1-iterative-work-stack.md). The latest mode allocates 126,464 arena bytes, retains a 4,096-byte allocator allowance, stores twenty packed frames inside the arena, and keeps its transcript fixed at 48 bytes. Its eight-seed maximum is 952 and no proof is produced. The immediate milestone is an independently designed bounded attack strategy rather than more tuning of this replay policy. Controlled-host measurement becomes gate-relevant only if an attacker emits canonical proofs.
