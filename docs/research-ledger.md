...
Stage C now has a [fail-closed online bounded probe](pow-v1-online-bounded-probe.md). It admits one logical half-scratch arena, executes exact prefixes, and independently commits to the first missing materialized read, but emits no digest. The immediate milestone is bounded reconstruction of that first miss and its historical machine state without loading the offline graph or schedule. A later exact attacker must account for retained values, identities, versions, checkpoints, register state, work queues, stack, allocator overhead, and peak resident memory within the ceiling.
