...
| Template autonomy | **OPEN** | One packaged labnet miner process builds and directly publishes two blocks outside the daemon, completes a Noise-authenticated Job Declaration/custom-job exchange for the first, and survives coordinator loss through direct fallback for the second. Independent interoperability, a noncustodial payout path, and adversarial multi-coordinator switching remain unproved. |
