...
Stage C now has a [fail-closed online bounded probe](pow-v1-online-bounded-probe.md), a [bounded first-reconstruction result](pow-v1-bounded-first-reconstruction.md), and a [bounded repeated-reconstruction result](pow-v1-bounded-repeated-reconstruction.md). The repeated evaluator recovers 109–137 successive missing values across eight standard cases before its 4,940-entry sparse replay table fills, while every replayed machine state matches independently. The immediate milestone is bounded checkpointed or recursive recovery beyond that exhaustion boundary. A later exact attacker must account for retained values, identities, versions, checkpoints, register state, work queues, stack, allocator overhead, and peak resident memory within the ceiling.
