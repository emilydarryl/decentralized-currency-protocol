...
Stage C now has a fail-closed online probe, first and repeated reconstruction results, and a [packed checkpoint pilot](pow-v1-packed-checkpoint-reconstruction.md). Bitmap-ranked packing raises standard replay capacity from 4,940 to 11,449 values and extends the fixed seed-zero prefix from iteration 2,609 to 6,615, but charges 180.8 GB of internal insertion movement. The immediate milestone is a block-gap, indexed-packed, or bounded time-checkpoint layout that preserves density with less update work. A later exact attacker must account for retained values, identities, versions, checkpoints, register state, work queues, stack, allocator overhead, and peak resident memory within the ceiling.
