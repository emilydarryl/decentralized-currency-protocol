...
| Template autonomy | **OPEN** | One packaged labnet miner process builds and directly publishes two blocks outside the daemon and survives accounting loss. The authenticated Job Declaration v0 behavior and nine semantic failure transcripts are frozen, but no binary network integration, second interoperable implementation, noncustodial payout path, or adversarial switching proof exists yet. |
