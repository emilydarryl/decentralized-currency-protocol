...
The [external attack challenge](pow-v1-external-attack-challenge.md) now freezes the independent submission contract. It separates a comparable five-million-operation screening track from an exact-completion track with no artificial work ceiling, commits public qualification cases, assigns fresh cases only after source freeze, forbids project CI from executing untrusted submissions, and states that artifact validation cannot prove a memory claim. The immediate milestone is a genuinely independent attack submission and accounting review. Controlled-host measurement becomes gate-relevant only if an eligible attacker emits canonical proofs.
