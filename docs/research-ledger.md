...
| Stratum V2 / Job Declaration labnet profile | Pin upstream `sv2-spec` commit `066971c7c750eded11b57aecd4ecdbd6e722c631`; require authenticated Noise NX for every coordinator connection; freeze the supported custom-job subset and direct fallback through nine semantic transcripts. The reference connection pins official Rust library commit `0b083ab9411a8a4c74421e90134b77410759b6df`. A second miner independently encodes and parses the supported payloads and block. One running miner now retains its own template through ordered failover across rejection, disconnect, stall, malformed state, downgrade, and total coordinator loss. Deterministic work claims bind target-valid receipts to miner-wallet payout scripts without a custodial key. | The pinned Noise mode authenticates the coordinator, not the miner; direct publication remains miner-controlled and off-chain use is not consensus-enforced. Claims are accounting only: decentralized replication, payout settlement, and public adversarial operation remain absent. | [Private-labnet profile v0](stratum-v2-job-declaration-labnet-profile-v0.md), [interoperability evidence](mining-interoperability-labnet.md), [payout and failover evidence](noncustodial-payout-failover-labnet.md) |
...
| Template autonomy | **OPEN** | Two independently written miners agree byte for byte and directly publish. One running miner preserves exact templates through five hostile coordinator behaviors, ordered alternate failover, and total coordinator loss. A local service emits deterministic noncustodial accounting claims, but decentralized share replication, actual settlement, and adversarial public-network evidence remain unproved. |
