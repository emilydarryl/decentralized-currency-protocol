...
In plain language, we are trying to attack our own mining design using a miner that keeps only half the normal scratch memory. The [hierarchical checkpoint-ladder pilot](pow-v1-hierarchical-checkpoint-ladder.md) replaces a twelve-bookmark linear search with four directly addressed time scales while keeping the explicit arena-resident work stack and five-million-operation ceiling. It is a stronger attack: the median exact prefix rises from 886 to 945.5 and the maximum from 952 to 982. All eight seeds still stop at the limit before completing the 98,304-step job, and none produces a proof. This is adversarial evidence, not a passed test: smarter attacks, exact proofs, controlled physical-memory measurements, and hardware comparisons remain outstanding. See [Mining Decentralization in Plain English](mining-decentralization-in-plain-english.md) for the full nontechnical explanation.
...
Stage C now has a fail-closed online probe, flat replay and dense-layout pilots, a time-checkpoint screen, recursive regeneration, target-aware dependency bundles, a total-operation bound, physical-memory accounting, an [explicit iterative work stack](pow-v1-iterative-work-stack.md), and a [hierarchical checkpoint ladder](pow-v1-hierarchical-checkpoint-ladder.md). The latest mode allocates 126,464 arena bytes, retains a 4,096-byte allocator allowance, stores twenty packed frames and sixty-four checkpoints inside the arena, and keeps its transcript fixed at 48 bytes. Its eight-seed maximum is 982 and no proof is produced. The immediate milestone is independent adversarial review and a second frozen bounded attack family rather than fitting more parameters to these eight public seeds. Controlled-host measurement becomes gate-relevant only if an attacker emits canonical proofs.
