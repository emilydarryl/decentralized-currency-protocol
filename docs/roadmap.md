...
The first bounded profile is frozen in the [private-labnet profile v0](stratum-v2-job-declaration-labnet-profile-v0.md): exact upstream revision, authenticated server identity, supported message subset, miner-created template fields, accepted flow, direct fallback, and nine canonical semantic transcripts. Issue #60 implements the reference Noise-authenticated binary connection, accepted custom job, and same-process direct fallback. Issue #61 adds a separately written miner, exact binary and block vectors, live cross-implementation tests, and one direct publication from each miner. Issue #62 adds deterministic accounting claims bound to miner-wallet payout scripts and one running miner switching across ordered coordinators through rejection, disconnect, stall, malformed state, downgrade, and final total coordinator loss. Issue #67 adds two local receipt replicas, deterministic outage recovery and agreement, and direct multi-output coinbase settlement. Issue #69 freezes the [sharechain private-lab profile v0](sharechain-v0.md): an exact entry format, fixed share target, trusted-round binding, accumulated-work fork choice, bounded finality and payout accounting, two separately written validators, and 15 adversarial histories. Issue #71 adds [authenticated three-process share synchronization](sharechain-sync-v0.md) with replay protection, hard message and orphan limits, restart persistence, partition and selective-relay recovery, and preserved equivocation evidence. The first part of issue #73 adds a [multi-host safety profile](sharechain-multihost-v1.md): signed ephemeral session preflight, exact admission accounting, peer-diversity selection, bounded long-partition recovery, portable signed equivocation evidence, and 37 deterministic checks on three loopback prefixes. The [routed namespace experiment v1](sharechain-routed-namespace-v1.md) now moves the signed layer onto the real peer socket, uses four processes and routed `/24` namespaces, persists replay rejection across restart, removes pairwise peer secrets from wire configuration, and retains deterministic CI evidence. The [four-operator kit v1](sharechain-operator-kit-v1.md) preserves the frozen three-peer rule while moving identity generation, manifest signing, private configuration, and final evidence signing into each operator's local package. CI proves the package fails closed; it is not a separate-host run. The remaining gate is an actual independently operated synchronization run, reviewed hybrid identity and key distribution, peer discovery and rotation, Sybil and eclipse evidence under hostile networks, and mature settlement rules.
